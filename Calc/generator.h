...
public:
	...
	std::unordered_map <std::string, uint16_t> priority = { {"!", 3}, {"&", 2}, {"|", 1}, {"^", 1}, {">", 0}, {"=", -1 } };
	...
	void eval_top() {
		//std::cout << "eval top with " << operators.top() << std::endl;
		std::visit(
			overload{
	...
	void push(std::string token) {
		...
		if (to_bool.contains(token)) {
			...
			//	std::cout << "closed bracket" << std::endl;
			do {
				eval_top();
			} while (operators.top() != "(");
			operators.pop();
			...
			//std::cout << "opened bracket" << std::endl;
			operators.push(token);
	...
	truth_table_gen(std::string expression) {
		for (auto r_token : expression) {
			if (r_token != ' ') {
				...
				else if (stack.to_bool.contains(std::string(1, r_token))) {
					std::string var;
